...
2.  **Streaming & Memory Safe Processing:** Alih-alih memuat seluruh isi berkas CSV berukuran megabyte ke memori (`file_get_contents` atau library berat), Job menggunakan metode low-level `fopen()` dan `fgetcsv()`. Konsumsi RAM container terbukti stabil di **bawah 5MB** selama proses impor.
...
Aplikasi ini sepenuhnya terisolasi di dalam Docker Container. Anda tidak perlu menginstal PHP, Composer, Redis, atau PostgreSQL secara lokal di komputer Anda.
...
> 💡 **Efek Samping Seeder:** Proses ini secara otomatis membuat akun Admin untuk keperluan pengujian dan men-generate berkas CSV asli berisi puluhan ribu data produk siap uji di dalam jalur aman `storage/app/private/products_real_sample.csv`.
...
Gunakan berkas berkontrak Postman yang telah disediakan di root direktori dengan nama: `dtk_import_queue_postman_collection.json`.

1. **Impor Koleksi:** Buka Postman, klik tombol **Import**, lalu pilih berkas json di atas.
2. **Otentikasi Awal:** Jalankan request **Login** menggunakan kredensial default hasil seeder:
   * Email: `admin@internal.com`
   * Password: `password123`
3. **Atur Variabel Token:** Salin string `access_token` dari respons sukses login. Buka tab **Variables** pada tingkat Collection Postman Anda, lalu tempel (*paste*) ke kolom `Current Value` pada variabel bernama `token`.
...
## 🔍 Sinkronisasi Log & Pemantauan Latar Belakang
...
Jika container gagal menulis file log atau memproses berkas, jalankan perintah perbaikan hak akses folder berikut di root proyek:
